...
<div id="rezgo_content_container" style="width:100%;">
	<iframe id="rezgo_content_frame" src="<?=REZGO_URL_BASE?>/frame_router.php?mode=<?=$_REQUEST['mode']?>&com=<?=$_REQUEST['com']?>&parent_url=<?=$wp_current_page?>&tags=<?=$_REQUEST['tags']?>&search_for=<?=$_REQUEST['search_for']?>&start_date=<?=$_REQUEST['start_date']?>&end_date=<?=$_REQUEST['end_date']?>&date=<?=$_REQUEST['date']?>&rezgo_page=<?=$_REQUEST['rezgo_page']?>&option=<?=$_REQUEST['option']?>&cid=<?=$_REQUEST['cid']?>&trans_num=<?=$_REQUEST['trans_num']?>" style="width:100%; height:900px; padding:0px; margin:0px;" frameBorder="0" scrolling="no"></iframe>
</div>
