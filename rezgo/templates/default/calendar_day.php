...
At least NUM are required.
      ...
      <a data-toggle="collapse" data-parent="#rezgo-select-option-<?=$option_num?>" data-target="#option_<?=$option_num?>" class="panel-heading panel-title rezgo-panel-option-link">
        <div class="rezgo-panel-option"><?=$option->option?>&nbsp;</div>
      </a>
			<div id="option_<?=$option_num?>" class="panel-collapse collapse<?=(((count($options) == 1 && $option_num == 1) || $_REQUEST['id'] == (int) $option->uid) ? ' in' : '')?>">
