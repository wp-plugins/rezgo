	  ...
	  <div class="col-md-4 col-sm-5 col-xs-12">
	    <div class="row">
	      ...
	      <div class="col-xs-7 col-sm-6">
              ...
              <a href="javascript:void(0);" id="social_url" data-toggle="popover" data-ajaxload="<?=$site->ajax_url?>/shorturl_ajax.php?url=<?= urlencode('http://'.$_SERVER['HTTP_HOST'].$site->base.'/details/'.$item->com.'/'.$site->seoEncode($item->item)) ?>"><i class="fa fa-share-alt-square" id="social_url_icon">&nbsp;</i></a>
			      </span>		      
		      </div>
	      </div>
          ...
          <div class="rezgo-calendar-single-head"><span class="rezgo-calendar-avail"><span>Availability&nbsp;for:</span></span> <strong><?=date((string) $company->date_format, $option['start_date'])?></strong></div>
            ...
            <script type="text/javascript">
              ...
              $(document).ready(function () {
                ...
                $.ajax({
                  url: '<?=$site->ajax_url?>/calendar_day.php?parent_url=<?=$site->base?>&com=<?=$item->com?>&date=<?=date('Y-m-d', $option['start_date'])?>&option_num=<?=$opt?>',
    ...
    $(document).ready(function () {
      ...
      $('.responsive-calendar').responsiveCalendar({
          ...
          onDayClick: function(events) { 
            ...
            this_class = events[this_date].class;
						...
						if (this_class == 'passed') {
							...
							$.ajax({
								url: '<?=$site->ajax_url?>/calendar_day.php?parent_url=<?=$site->base?>&com=<?=$item->com?>&date=' + this_date,
								context: document.body,
								success: function(data) {
          ...
          /*onDayHover: function(events) { 
            ...
            if (this_class == '') {
					...
					onMonthChange: function(events) { 
						...
						$.ajax({
							url: '<?=$site->ajax_url?>/calendar_month.php?uid=<?=$item->uid?>&com=<?=$item->com?>&date=' + rezNewMonth,
							context: document.body,
							success: function(data) {
		  ...
		  <? if ( ($non_single_dates > 0 || $single_dates > 10) && $cal_day_set === TRUE ) { ?>
			...
			$.ajax({
				url: '<?=$site->ajax_url?>/calendar_day.php?parent_url=<?=$site->base?>&com=<?=$item->com?>&date=<?=$open_cal_day?>&id=<?=$_REQUEST['option']?>',
				context: document.body,
				success: function(data) {
					$('.rezgo-date-selector').html(data).css('opacity', '1');
					$('.rezgo-date-options').fadeIn('slow');	
